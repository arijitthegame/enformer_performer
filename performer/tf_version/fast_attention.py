...
def softmax_kernel_transformation(
    data, is_query, projection_matrix, numerical_stabilizer=0.000001
):
...
def favor_attention(
    query, key, value, kernel_transformation, causal, projection_matrix
):
...
class FixedPositionalEmbedding(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        self.inv_freq = 1.0 / (
            10000
            ** (tf.range(start=0, limit=self.dim, delta=2, dtype="float32") / self.dim)
        )
        self.position = tf.range(
            start=0, limit=self.max_seq_len, delta=1, dtype="float32"
        )
        self.sinusoid_inp = tf.einsum("i,j->ij", self.position, self.inv_freq)
        self.emb = tf.concat(
            (tf.math.sin(self.sinusoid_inp), tf.math.cos(self.sinusoid_inp)), axis=-1
        )
    ...
    def call(
        ...
        if self.use_mask_pos is True:
            create_kernel = partial(
                softmax_kernel_transformation, projection_matrix=projection_matrix
            )
            ...
            k = rearrange(
                k, "b h n d -> b h d n", h=h
            )  # (batch, head, dim_head, seq_len) ([1, 8, 1000, 16])
            ...
            attention_output = rearrange(
                attention_output, "b n h d -> b n h d"
            )  # this step differs from the pytorch implementation
        ...
        if self.use_mask_pos is False:
            attention_output = favor_attention(
                q, k, v, self.kernel_transformation, self.causal, projection_matrix
            )
            attention_output = rearrange(
                attention_output, "b h n d -> b n h d"
            )  # this step differs from the pytorch implementation
...
class SelfAttention(Attention):
    ...
    def call(
        ...
        return super(SelfAttention, self).call(
            query_input, query_input, rpe, training, cache, decode_loop_step
        )
...
class PerformerBlock(tf.keras.layers.Layer):
    ...
    def call(self, x, rpe=None, **kwargs):
        ...
        x = x + self.dropout(self.attention(x, x, rpe=rpe, **kwargs))
...
class PerformerEncoder(tf.keras.layers.Layer):
    def __init__(
        ...
        if self.use_mask_pos is True:
            self.relative_positional_bias = tf.Variable(
                tf.random.uniform((self.n_heads, 2 * self.rel_pos_bins - 1))
            )
    ...
    def call(self, x, rpe=None, **kwargs):
        ...
        if self.use_mask_pos is True:
            ...
            if L <= self.rel_pos_bins:
                rpe = tf.concat(
                    (
                        tf.expand_dims(self.relative_positional_bias[:, 0], axis=1),
                ...
                rpe = tf.concat(
                    [
                        tf.repeat(
                            tf.expand_dims(self.relative_positional_bias[:, 0], axis=1),
                            repeats=L - self.rel_pos_bins + 1,
                            axis=1,
                        ),
                        self.relative_positional_bias,
                        tf.repeat(
                            tf.expand_dims(
                                self.relative_positional_bias[:, -1], axis=1
                            ),
                            repeats=L - self.rel_pos_bins,
                            axis=1,
                        ),
                    ],
                    axis=1,
