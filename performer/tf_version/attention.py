...
class TransformerBlock(tf.keras.Model):
    ...
    def __init__(
        ...
        self.mha_ln = kl.LayerNormalization(axis=-1,
                                            scale=True,
                                            center=True,
                                            beta_initializer="zeros",
                                            gamma_initializer="ones")
        self.mha = MultiheadAttention(**attention_kwargs)
        self.mha_dropout = kl.Dropout(dropout_rate)

        self.mlp_ln = kl.LayerNormalization(axis=-1,
                                            scale=True,
                                            center=True,
                                            beta_initializer="zeros",
                                            gamma_initializer="ones")
        self.mlp_linear1 = kl.Dense(channels * 2)
        self.mlp_dropout1 = kl.Dropout(dropout_rate)
        self.mlp_linear2 = kl.Dense(channels)
        self.mlp_dropout2 = kl.Dropout(dropout_rate)
...
class MultiheadAttention(tf.keras.Model):
    ...
    def build(self, input_shape):
        ...
        num_key_features = input_shape[-1]
        num_value_features = (
        ...
        self._q_layer = self.add_weight(
            name='q_layer',
            shape=[self.num_heads, num_query_features, self._value_size],
            initializer=self.kernel_initializer,
            regularizer=self.kernel_regularizer,
            constraint=self.kernel_constraint,
        )
        self._k_layer = self.add_weight(
            name='k_layer',
            shape=[self.num_heads, num_query_features, self._value_size],
            initializer=self.kernel_initializer,
            regularizer=self.kernel_regularizer,
        ...
        self._embedding_layer = kl.Dense(
            embedding_size,
            name='embedding_layer',
            kernel_initializer=w_init)
        
        if self._relative_positions:
            self._r_k_layer = kl.Dense(
                key_proj_size,
                name='r_k_layer',
                use_bias=False,
    ...
    def call(self,
        ...
        q = tf.einsum("...NI , HIO -> ...NHO", inputs, self._q_layer)
        k = tf.einsum("...MI , HIO -> ...MHO", inputs, self._k_layer)
        v = tf.einsum("...MI , HIO -> ...MHO", inputs, self._v_layer)
        ...
        attended_inputs = snt.reshape(
            output_transpose, [embedding_size], preserve_dims=2)
