...
class SineSPE(nn.Module):
    ...
    def forward(self, shape, num_realizations=None):
        ...
        omega_q = tf.stack([tf.math.cos(phases_q), tf.math.sin(phases_q)], dim=-1).view(
        ...
        omega_k = tf.stack([tf.math.cos(phases_k), tf.math.sin(phases_k)], dim=-1).view(
            1, self.num_heads, self.in_features, max_len, 2*self.num_sines
        )
        ...
        z = tf.random.normal(
            ...
            num_realizations,
            device=self.freqs.device) / tf.math.sqrt(self.num_sines * 2)
...
class SPEFilter(nn.Module):
    ...
    def __init__(
        ...
        if gated:
            ...
            self.register_parameter('gate', tf.Variable(
                tf.random.normal(code_shape)
            ))
