...
Easist thing will do length of the list a hyperparameter. 
"""
...
class Enformer(tf.keras.Model):
    ...
    def call(self, inputs: tf.Tensor, training: bool) -> Dict[str, tf.Tensor]:
...
if __name__ == "__main__":
    ...
    model = Enformer(
        channels=1536, num_heads=2, num_transformer_layers=2, pooling_type="max"
    )
    print(model(x, training=True))

    y = tf.random.uniform(shape=(1, 131072, 4))
    model = Enformer(
        channels=1536, num_heads=2, num_transformer_layers=2, pooling_type="attention"
    )
